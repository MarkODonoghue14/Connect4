...
/**
 * CS152 Lab 5 -- Connect Four
 *
 * Class to play a game of Connect 4.
 * Computer plays black, human plays red.
 *
 * Student name: Merry Hospelhorn
 */
...
/*    	if (board[5][ROWS] == (NONE)){
            return false;
    	}else{
		return true;}}*/
        for (int y = 0; y < COLUMNS; y++){
            if (board[ROWS-1][y] == NONE){
                return false;
            }
        }
        return true;
    ...
    public static Color winAt(Color[][] board, int r, int c,
        ...
        Color myPiece = board [r][c];
        for (int i = 0; i < 4 && c >= 0 && c < COLUMNS; i++){
    ...
    public static Color winner(Color[][] board) {
        ...
        for (int i = 0; i < 2; i++){
            for (int j = 0; j < 3; j++){
                ...
                result = winAt(board, i,j,0,1);
                if (result != NONE){
                    return result;
                }
                result = winAt(board, i,j,1,0);
                if (result != NONE){
                    return result;
                }
            }
